/* Code from bonzaiferroni */

"use strict";
const REPORT_CPU_THRESHOLD = 200;
    ...
    findAllowedRooms(origin, destination, options = {}) {
        ...
        let ret = Game.map.findRoute(origin, destination, {
            routeCallback: (roomName) => {
                ...
                if (!options.allowInvaderRoom && this.memory.invasionRooms[roomName])
                {
                    return Number.POSITIVE_INFINITY;
                }
                return 2.5;
    ...
    travelTo(creep, destination, options = {}) {
        ...
        if (creep.room.controller) {
            if (creep.room.controller.owner && !creep.room.controller.my) {
                if (creep.room.find(FIND_STRUCTURES, {filter : (s) => s.structureType != STRUCTURE_CONTROLLER}).length == 0)
                {
                ...
                else
                {
                    ...
                    Memory.empire.hostileRooms[creep.room.name] = creep.room.controller.level;
                }
            }
            else {
                this.memory.hostileRooms[creep.room.name] = undefined;
        ...
        else if (!creep.room.controller && creep.room.find(FIND_HOSTILE_CREEPS, {filter : (c) => c.owner == 'Source Keeper'}).length > 0){
            this.memory.hostileRooms[creep.room.name] = 10;
        }
